...
Definir orçamento semanal e limite de alerta (em %).
Registrar nova despesa (dia da semana, categoria, valor).
Listar todas as despesas registradas (formato tabular simples com f-strings).
Resumo por categoria (mostrar categoria, total gasto e % do orçamento total).
Verificar situação do orçamento (total gasto, restante, se ultrapassou ou está abaixo do limite de alerta).
Estatísticas dos dias (qual dia teve maior gasto, qual teve menor, média diária).
Resetar todos os dados (limpar listas após confirmação).
Sair.
